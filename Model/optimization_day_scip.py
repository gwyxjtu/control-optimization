...
def opt_day(parameter_json, load_json, begin_time, time_scale, storage_begin_json, storage_end_json):
    ...
    for t in range(period):
        ...
        model_scip.addCons(
            g_ghp_de[t] + g_eb_de[t] + g_ahp[t] + g_fc_de[t] + g_ht[t] == g_load[t]
        )
        model_scip.addCons(h_pur[t] == m_h_fc[t])
        # model_scip.addCons(
        #     m_ghp_lb + m_eb_lb + m_ahp_lb + m_fc_lb + m_ht_lb == m_de_lb
        # )
        # model_scip.addCons(m_ghp[t] == m_ghp_lb)
        # model_scip.addCons(m_eb[t] == m_eb_lb)
        # model_scip.addCons(m_ahp[t] == m_ahp_lb)
        # model_scip.addCons(m_fc[t] == m_fc_lb)
        # model_scip.addCons(m_ht[t] == m_ht_lb)
        # model_scip.addCons(m_de[t] == m_de_lb)
        ...
        model_scip.addCons(p_pur[t] <= z_pur[t] * M)
        model_scip.addCons(g_ghp[t] == z_ghp_ht[t] * g_ghp_ht[t] + z_ghp_de[t] * g_ghp_de[t])
        model_scip.addCons(z_ghp_de[t] + z_ghp_ht[t] == z_ghp[t])

        model_scip.addCons(g_eb[t] == z_eb_ht[t] * g_eb_ht[t] + z_eb_de[t] * g_eb_de[t])
        model_scip.addCons(z_eb_de[t] + z_eb_ht[t] <= 1)

        model_scip.addCons(g_fc[t] == z_fc_ht[t] * g_fc_ht[t] + z_fc_de[t] * g_fc_de[t])
        model_scip.addCons(z_fc_de[t] + z_fc_ht[t] <= 1)

        model_scip.addCons(g_ghp_ht[t] + g_eb_ht[t] + g_fc_ht[t] <= z_ht_sto[t] * M)
        model_scip.addCons(g_ht[t] <= (1 - z_ht_sto[t]) * M)

        # model_scip.addCons(g_ghp[t] == eta_ghp[t] * z_ghp[t] * p_ghp_max)
        model_scip.addCons(g_ghp[t] == eta_ghp * z_ghp[t] * p_ghp_max)
    ...
    #     model_scip.addCons(eta_ghp[t]==2+0.1209*t_gtw_out[t])
    #     model_scip.addCons(g_gtw[t]==c_water*m_gtw*(t_gtw_out[t]-t_gtw_in[t]))
    #     model_scip.addCons(t_gtw_out[t]==0.2*(t_gtw_out[t]-t_b[t])+t_b[t])
    # model_scip.addCons(t_b[0]==10.5-(1000/(2*np.pi*2.07*200*192))*(g_gtw[0])*g_func[0])  # 地热井进水温度初始值
    # # model.addConstr(t_b[0]==10.5-(1000/(2*np.pi*2.07*200*192))*(g_gtw[0])*g_func[0])
    # for t in range(1,period):
    #     model_scip.addCons(t_b[t]==10.5-(1000/(2*np.pi*2.07*200*192))*opt.quicksum((g_gtw[j]-g_gtw[j-1])*g_func[t-j] for j in range(t+1)))
    #     # model.addConstr(t_b[t]==10.5-(1000/(2*np.pi*2.07*200*192))*gp.quicksum((g_gtw[j]-g_gtw[j-1])*g_func[t-j] for j in range(t+1)))

    # EB
    for t in range(period):
        model_scip.addCons(g_eb[t] == eta_eb * p_eb[t])
        model_scip.addCons(g_eb[t]*z_eb_de[t]==c_water*m_eb_lb*(t_eb[t]-t_de[t]))
        model_scip.addCons(p_pump_eb[t] == eta_pump_eb * m_eb_lb)
    # AHP
    # NOTE: 修改空气源热泵的效率计算
        # 一共有5个计算分区，顺序与表格一致
    # for t in range(period):
    #     if t_env[t] < -15:
    #         model_scip.addCons(eta_ahp[t] == eta_ahp_base[0] + k_t_env[0] * t_env[t] + k_t_ahp[0] * t_ahp[t])

    #         # model.addConstr(eta_ahp[t] == eta_ahp_base[0] + k_t_env[0] * t_env[t] + k_t_ahp[0] * t_ahp[t])
    #     elif t_env[t] < 0 and t_env[t] >= -15:
    #         model_scip.addCons(eta_ahp[t] == eta_ahp_base[1] + k_t_env[1] * t_env[t] + k_t_ahp[1] * t_ahp[t])
    ...
    #     #     model.addConstr(eta_ahp[t] >= eta_ahp_base[3] + k_t_env[3] * t_env[t] + k_t_ahp[3] * t_ahp[t] - (1-z_ahp_cop[t])*M)
    #           model_scip.addCons(eta_ahp[t] == eta_ahp_base[2] + k_t_env[2] * t_env[t] + k_t_ahp[2] * t_ahp[t])

    #     else:
    #         model_scip.addCons(eta_ahp[t] == eta_ahp_base[4] + k_t_env[4] * t_env[t] + k_t_ahp[4] * t_ahp[t])

    # model.addConstrs(eta_ahp[t] == eta_ahp_base + k_t_env * t_env[t] + k_t_ahp * t_ahp[t] for t in range(period))
        # HACK: 空气源热泵使用静态COP
        model_scip.addCons(g_ahp[t] == eta_ahp * p_ahp[t])
        model_scip.addCons(g_ahp[t] == c_water * m_ahp_lb * (t_ahp[t] - t_de[t]))
        model_scip.addCons(p_pump_ahp[t] == eta_pump_ahp * m_ahp_lb)

    # FC
    # NOTE: 修改了FC热计算

    for t in range(period):
        model_scip.addCons(p_fc[t] == eta_fc_p * m_h_fc[t])
    for t in range(period):
        # HACK: 使用200-400kW的燃料电池电热关系函数计算产热
        model_scip.addCons(g_fc[t] == k_g_p_400 * p_fc[t] + b_g_p_400)
        model_scip.addCons(g_fc[t] * z_fc_de[t] == c_water * m_fc_lb * (t_fc[t] - t_de[t]))
        model_scip.addCons(p_pump_fc[t] == eta_pump_fc * m_fc_lb)

    # HT
    # NOTE: 文档中该公式是否有问题？
    for t in range(period-1):
        model_scip.addCons(z_ghp_ht[t] * g_ghp_ht[t] + z_eb_ht[t] * g_eb_ht[t] + z_fc_ht[t] * g_fc_ht[t] - g_ht[t]
    ...
    #                  == c_water * m_ht_sto * (t_ht_sto[t + 1] - t_ht_sto[t]) + eta_ht_loss * (t_ht_sto[t] - t_env[t])
    #                  for t in range(period - 1))
    model_scip.addCons(z_ghp_ht[-1]*g_ghp_ht[-1] + z_eb_ht[-1]*g_eb_ht[-1] + z_fc_ht[-1]*g_fc_ht[-1] - g_ht[-1]
                  == c_water * m_ht_sto * (t_ht_sto[0] - t_ht_sto[-1]) + eta_ht_loss * (t_ht_sto[-1] - t_env[23]))
    # NOTE: 建模可否省略为 g^{HW} = c * m^{HW} * (t^{HW} - t^{DE})？可以
    for t in range(period):
        model_scip.addCons(g_ht[t] == c_water * m_ht_lb * (t_ht[t] - t_de[t]))

    # model.addConstrs(p_pump_ht[t] == eta_pump_ht * m_ht[t] for t in range(period))
    # BS
    for t in range(period-1):
        model_scip.addCons(p_bs_sto[t + 1] - p_bs_sto[t] == p_bs_ch[t] - p_bs_dis[t])
    model_scip.addCons(p_bs_sto[0] - p_bs_sto[-1] == p_bs_ch[-1] - p_bs_dis[-1])
    # PV
    for t in range(period):
        model_scip.addCons(p_pv[t] == eta_pv * p_pv_max * pv_generation[t])
    # model.addConstrs(p_pv[t] == eta_pv * p_pv_max * pv_generation[t] for t in range(period))
    # PIPE
    for t in range(period):
        model_scip.addCons(g_load[t] == c_water * m_de_lb * (t_supply[t] - t_de[t])) #+ eta_pipe_loss * (t_supply[t] - t_env[t]))
        model_scip.addCons(p_pump_pipe[t] == eta_pump_pipe * m_de_lb)
    ...
    # opex
    for t in range(period):
        model_scip.addCons(opex_t[t] == hydrogen_price * h_pur[t] + lambda_ele_in[t] * p_pur[t])
    # model.addConstrs(opex_t[t] == hydrogen_price * h_pur[t] + lambda_ele_in[t] * p_pur[t] for t in range(period))
    model_scip.addCons(opex == opt.quicksum(opex_t[t] for t in range(period)))  # 总运行成本
    ...
    # 重新构建输出
    dict_control = {
        "opex": model_scip.getVal(opex),
        "opex_t": [model_scip.getVal(opex_t[t]) for t in range(period)],
        "z_pur": [model_scip.getVal(z_pur[t]) for t in range(period)],
        "z_ghp": [model_scip.getVal(z_ghp[t]) for t in range(period)],
        "z_ghp_de": [model_scip.getVal(z_ghp_de[t]) for t in range(period)],
        "z_ghp_ht": [model_scip.getVal(z_ghp_ht[t]) for t in range(period)],
        "z_eb_de": [model_scip.getVal(z_eb_de[t]) for t in range(period)],
        "z_eb_ht": [model_scip.getVal(z_eb_ht[t]) for t in range(period)],
        "z_fc_de": [model_scip.getVal(z_fc_de[t]) for t in range(period)],
        "z_fc_ht": [model_scip.getVal(z_fc_ht[t]) for t in range(period)],
        "z_ht_sto": [model_scip.getVal(z_ht_sto[t]) for t in range(period)],
        "p_pv": [model_scip.getVal(p_pv[t]) for t in range(period)],
        "p_pur": [model_scip.getVal(p_pur[t]) for t in range(period)],
        ...
        "p_pump_ghp": [model_scip.getVal(p_pump_ghp[t]) for t in range(period)],
        "p_eb": [model_scip.getVal(p_eb[t]) for t in range(period)],
        "g_eb": [model_scip.getVal(g_eb[t]) * p_eb_max for t in range(period)], 
        "g_eb_ht": [model_scip.getVal(g_eb_ht[t]) for t in range(period)],
        "g_eb_de": [model_scip.getVal(g_eb_de[t]) for t in range(period)],
        "t_eb": [model_scip.getVal(t_eb[t]) for t in range(period)],
        "p_ahp": [model_scip.getVal(p_ahp[t]) for t in range(period)],
        "g_ahp": [model_scip.getVal(g_ahp[t]) for t in range(period)],
        "t_ahp": [model_scip.getVal(t_ahp[t]) for t in range(period)],
        "m_h_fc": [model_scip.getVal(m_h_fc[t]) for t in range(period)],
        "p_fc": [model_scip.getVal(p_fc[t]) for t in range(period)],
        "g_fc": [model_scip.getVal(g_fc[t]) for t in range(period)],
        "g_fc_ht": [model_scip.getVal(g_fc_ht[t]) for t in range(period)],
        "g_fc_de": [model_scip.getVal(g_fc_de[t]) for t in range(period)],
        "t_fc": [model_scip.getVal(t_fc[t]) for t in range(period)],
        "g_ht": [model_scip.getVal(g_ht[t]) for t in range(period)],
        "t_ht_sto": [model_scip.getVal(t_ht_sto[t]) for t in range(period)],
        "t_ht": [model_scip.getVal(t_ht[t]) for t in range(period)],
        "p_bs_sto": [model_scip.getVal(p_bs_sto[t]) for t in range(period)],
        "p_bs_ch": [model_scip.getVal(p_bs_ch[t]) for t in range(period)],
        "p_bs_dis": [model_scip.getVal(p_bs_dis[t]) for t in range(period)],
